...
def master(filename):
    PATH = '\\\\amer.pfizer.com\\pfizerfiles\\Research\\LAJ\\oru-tcb\\ChemBio\\HUFFMP\\misc\\bESI\\bESI TMT2-test\\'
    clock = timer()
    ...
    contamsfile = ('\\\\amer.pfizer.com\\pfizerfiles\\Research\\LAJ\\oru-tcb\\ChemBio\\HUFFMP\\misc\\'+
                   'contaminants3.fasta')
    proteomefile = ('\\\\amer.pfizer.com\\pfizerfiles\\Research\LAJ\\oru-tcb\\ChemBio\\HUFFMP\\results\\'+
                    'homo sapiens.fasta')
...
for x in range(40,71):
    print(f'Analyzing: PFAS_YLpep_{str(x)}.xlsx')
    master(f'PFAS_YLpep_{str(x)}.xlsx')
