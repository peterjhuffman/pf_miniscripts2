...
tabs = [10, 1, 0.1, 0]
logtab = [np.log10(x+0.000000001) for x in tabs]
...
for target in GENEs:
    ...
    gvals = [x/gvals[-1] for x in gvals]
#    print(gvals)
...
#    guess = [1, 2, 1, 0]
#    h_popt, h_pcov = curve_fit(sigmoid, logtab, gvals, p0=guess)
##    corr = round(np.diag(h_pcov).sum(),3)
#    hla = (np.log((h_popt[0]/0.5)-(h_popt[3]+1)))/h_popt[2] - h_popt[1]
#    hl = 10**hla
...
hdf_tf.to_excel(filepath+'hlog_modeled2.xlsx', index=False)
