...
def pval_from_peptides(aux,pep,target, ctrl):
    slices = [dfi[dfi['Master Protein Accessions']==aux] for aux in aux]
    ctrl_vals = [dfg[ctrl].mean(axis=1).values for dfg in slices]
    comp_vals = [dfgi[target].mean(axis=1).values for dfgi in slices]

    pvals = [-np.log10(ttr(ctrl_val, comp_val)[1]) for ctrl_val,comp_val in zip(ctrl_vals, comp_vals)]
...
filename = input('Peptide Export Filename? : ')
if filename[0]=='f':
    filename = filename[5:]
fileSuffix = pathlib.Path(filename).suffix
foldersav = pathlib.Path(filename).stem
...
for i, loc in zip(ctrl_loc_loc, ctrl_loc):
    if loc != '!':
        ...
        df_pval = pval_from_peptides([aux for aux in df_aux['Accession']], dfi, ab_cols[col_bounds[0]:col_bounds[1]], ab_cols[ctrl_bounds[0]:ctrl_bounds[1]])
