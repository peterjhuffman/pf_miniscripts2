...
#df.loc[mask_p&plus_mask, 'label'] = df[mask_p&plus_mask]['Gene Symbol']
df.loc[mask_p&plus_mask, 'metamark'] = '-'

#df.loc[mask_p&minus_mask, 'label'] = df[mask_p&minus_mask]['Gene Symbol']
df.loc[mask_p&minus_mask, 'metamark'] = '+'

df[df['metamark']=='-']['Gene Symbol'].to_csv(f"{'.'.join(datasource.split('.')[:-1])}_metaminus.csv", index=False)
df[df['metamark']=='+']['Gene Symbol'].to_csv(f"{'.'.join(datasource.split('.')[:-1])}_metaplus.csv", index=False)
...
#newfile = f"{'.'.join(datasource.split('.')[:-1])}2.csv"
#
print('Saving file.')
